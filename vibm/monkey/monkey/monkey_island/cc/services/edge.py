...
class EdgeService:
    ...
    def get_displayed_edge_by_id(edge_id, for_report=False):
        ...
        return EdgeService.edge_to)displayed_edge(edge, for_report)

    @staticmethod
    def get_displayed_edges_by_to(to, for_report=False):
        edges = mongodb.db.edge.find({"to": ObjectId(to)})
    ...
    def insert_edge(from_id, to_id):
        ...
        return mongo.db.edge.find_one({"_id": edge_insert_result.insert_id})

    @staticmethod
    def get_or_create_edge(edge_from, edge_to):
        tunnel_edge = mongo.db.edge.find_one({
            "from": edge_from, 
            "to": edge_to
        })
    ...
    def get_infected_monkey_island_pseudo_edges():
        ...
        existing_ids = [x["from"] for x in mongo.db.edge.find({
            "to": monkey["_id"]
        })]
        monkey_ids = [x["_id"] for x in mongo.db.monkey.find({}) if (
            "tunnel" not in x) and (x["_id"] not in existing_ids) and (x["_id"] != monkey["_id"])]
    ...
    def get_edge_label(edge):
        ...
        if edge["to"] == ObjectId("000000000000000000000000"):
            ...
            to_id = NodeService.get_node_label(get_monkey_by_id(edge["to"]))
            if to_id is None:
