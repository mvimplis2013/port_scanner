...
Provides an :class:`ElasticSearchTarget` and a :class:`CopyIndex` template task.

Modeled after :class:`finestrino`contrib.rdbms.CopyToTable`.

A minimal example (assuming that elasticsearch is running on ``localhost:9200``):
...
... code-block:: python 
    ...
    if __name__ == '__main__':
       task = ExampleIndex()
       finestrino.build([task], local_scheduler=True)
...
index:
* 1 ... default, keep all updates
* 0 ... only remember the most recent update to the index

There are two finestrino configuration options:
...
class ElasticsearchTarget(finestrino.Target):
    """
    Target for a resource in Elasticsearch.
    """
    marker_index = finestrino.configuration.get_config().get('elasticsearch', 'marker-index', 'update_log')
    marker_doc_type = finestrino.configuration.get_config.get('elasticsearch', 'marker-doc-type', 'entry')

    def __init__(self, host, port, index, doc_type, update_id, marker_index_hist_size=0, http_auth=None, timeout=10, 
    ...
    def touch(self):
        ...
        The document id would be sufficient but for documentation 
        we index the parameters `update_id`, `target_index` and `date` as well.
        """
        self.create_marker_index()
        self.es.index(index=self.marker_index, doc_type=self.marker_doc_type, id=self.marker_index_id(),
            body={
...
class CopyToIndex(finestrino.Task):
    """
