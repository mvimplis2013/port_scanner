...
from finestrino.target import AtomicLocalFile, FileSystem, FileAlreadyExists, NotADirectory, MissingParentDirectory, FileSystemTarget

from finestrino.format import get_default_format, FileWrapper
...
class atomic_file(AtomicLocalFile):
    ...
    def generate_tmp_path(self, path):
        return path + '-finestrino-tmp-%09d' % random.randrange(0, 1e10)
...
class LocalTarget(FileSystemTarget):
    ...
    def __init__(self, path=None, format=None, is_tmp=False):
        ...
        if not path:
            ...
            path = os.path.join(tempfile.gettempdir(), "finestrino-tmp-%09d" % random.randint(0, 999999999))
