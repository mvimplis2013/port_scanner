...
class execution_sumary(finestrino.Config):
    summary_length = finestrino.IntParameter(default=5)
...
def _partition_tasks(worker):
    ...
    set_tasks["already_done"] = {task for (task, status, ext) in task_history if status == 'DONE' 
        and task not in pending_tasks and task not in set_tasks["completed"]}
...
def _depth_first_search(set_tasks, current_task, visited):
    ...
    if current_task in set_tasks["still_pending_not_ext"]:
        ...
        if not upstream_failure and not upstream_missing_dependency and not upstream_run_by_other_worker and \
                not upstream_scheduling_error and \
...
def _get_str(task_dict, extra_indent):
    ...
    """
    summary_length = execution_sumary().summary_length
    ...
    for task_family in task_names:
        ...
        if len(tasks[0].get_params()) == 0:
            line = prefix + "- {0} {1}()".format(len(tasks), str(task_family))
                    ...
                    (len(tasks) == 2 and len(tasks[0].get_params()) > 1 and \
                    (_get_len_of_params(tasks[0]) > 40 or len(str(tasks[0])) > 100)):
            """
