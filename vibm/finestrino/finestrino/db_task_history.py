...
See :ref:`TaskHistory` for information about how to turn  out the tak history feature.
"""
#
# Description: Added codes for visualization of how long each task takes running-time until 
# it reaches the next status (failed ir done).
# At "{base_url}/tasklist", all completed (failed or done) tasks are shown.
# At "{base_url}/tasklist". the user can select one specific task to see how its running-time has changed over time.
# At "{base_url}/tasklist/{task_name}", it visualizes a multi-bar graph that represebnts the changes of the running-time 
#   for a selected task up to the next status (failed ot done).
# This visualization let us know how the running-time of the specific task has changed over time.
# 
...
import finestrino 
from finestrino import task_history
from finestrino import configuration
from finestrino.task_status import PENDING, DONE, FAILED, RUNNING
...
class DbTaskHistory(task_history.TaskHistory):
    """
    Task Histort that writes to a database using sqlalchemy.
    Also has methods for db queries.
    """
    ...
    def find_all_by_name(self, task_name, session=None):
        """
        Find all tasks with given name
        """
    ...
    def find_all_runs(self, session=None):
        """
        Returns all tasks that have been updated.
        """
        with self._session(session) as session:
    ...
    def find_all_events(self, session=None):
        """
        Returns all running/failed/done events.
        """
        with self._session(session) as session:
...
class TaskParameter(Base):
    """
    Table to track finestrino.Parameter()s of a Task
    """
...
class TaskEvent(Base):
    """ 
    Table to track when a task is scheduled, started, finished and failed.
    """
...
class TaskRecord(Base):
    """
    Base table to track information about a finestrino.Task.
    """
