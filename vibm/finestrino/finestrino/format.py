...
class InputPipeProcessWrapper(object):
    def __init__(self, command, input_pipe=None):
        ...
        if input_pipe is not None:
            try:
                ...
                f = tempfile.NamedTemporaryFile('wb', prefix='finestrino-process_tmp', delete=False)
    ...
    def _finish(self):
        # we need to close this first before input_pipe to get all SIGPIPE messages correctly
...
class MixedUnicodeBytesWrapper(BaseWrapper):
    ...
    def writelines(self, lines):
        self._stream.writelines((self._covert(line) for line in lines))
