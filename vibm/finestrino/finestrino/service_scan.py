...
def nmap_scan(ip_address, output_directory, dns_server, quick, no_udp_service_scan):
    ...
    if dns_server:
        ...
        UDPSCAN = "nmap -vv -Pn -A -sC -sU -T 4 --top-ports 200 --max-retries 0 --dns-servers %s -oN '%s/%sU.nmap' -oX '%s/%sU_nmap_scan_import.xml' %s" % (dns_server, output_directory, ip_address, ip_address)
    else:
