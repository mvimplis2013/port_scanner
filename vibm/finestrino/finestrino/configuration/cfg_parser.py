"""
finestrino.configuration provides some convenience wrappers around Python's 
ConfigParser to get configuration options from "config" files.

The default location for configuration files is 
  finetsrino.cfg (or client.cfg) in current working directory,
...
class InterpolationMissingEnvvarError(InterpolationError):
    ...
    Arguments:
        InterpolationError {[type]} -- [description]
    """
...
class EnvironmentInterpolation(Interpolation):
    """Custom interpolation which allows values to refer to environment 
    variables using the ``${ENVVAR}`` syntax.
