...
class ExternalStreams( finestrino.ExternalTask ):
    """
    Example of a possible external data dump.

    To depend on external targets (typically at the top of your dependency graph), 
    you can define an ExternalTask like this ...
    """
    ...
    def output(self):
        ...
        """
        return finestrino.contrib.hdfs.HdfsTarget(self.date.strftime('data/streams_%Y_%m_%d.tsv'))

class Streams( finestrino.Task ):
    """
    ...
    def output(self):
        ...
        :rtype: object (:py:class:`finestrino.target.Target`) 
        """
        return finestrino.LocalTarget(self.date.strftime('data/streams_%Y_%m_%d_faked.tsv'))

class StreamsHdfs(Streams):
    """
    This task performs the same work as :py:class.`~.Streams.run` and 
    overrides :py:meth:`~.Streams.output` so redefining HSFS as its target.
    """
    def output(self):
        """
        Returns the target output for this task.
        ...
        """
        return finestrino.contrib.hdfs.HdfsTarget(self.date.strftime('data/streams_%Y_%m_%d_faked.tsv'))

class AggregateArtists(finestrino.Task):
    """
    ...
    def output(self):
        ...
        """
        return finestrino.LocalTarget("data/artist_streams_{}.tsv".format(self.date_interval))
...
class AggregateArtistsHadoop(finestrino.contrib.hadoop.JobTask):
    """
    ...
    def output(self):
        ...
        :rtype: object (:py:class:`luigi.target.Target`)
        """
        return finestrino.contrib.hdfs.HdfsTarget(
            "data/artist_streams_%s.tsv" % self.date_interval,
            format=finestrino.contrib.hdfs.PlainDir
        )
...
class Top10Artists(finestrino.Task):
    """
    ...
    date_interval = finestrino.DateIntervalParameter()
    use_hadoop = finestrino.BoolParameter()
    ...
    def output(self):
        ...
        :rtype: object (:py:class:`luigi.target.Target`)
        """
        return finestrino.LocalTarget("data/top_artists_%s.tsv" % self.date_interval)
...
class ArtistToplistToDatabase(finestrino.contrib.postgres.CopyToTable):
    """
