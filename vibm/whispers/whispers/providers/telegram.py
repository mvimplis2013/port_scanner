...
class TelegramUpdates(TelegramMixin, ProviderResource):
    ...
    def _get_resource(self, data: dict) -> list:
        url = self.base_url.format(token=data["token"]) + self.updates_endpoints
        response, errors = requests.get(url, path_to_errors=self.path_to_errors)
        if errors:
            raise ResourceErrro( 
                errors=errors, resource=self.resource_name, 
                provider=self.name, data=data, response=response,
            )
...
class Telegram(TelegramMixin, Provider):
    ...
    _schema = {
        ...
        "properties": {
            ...
            "chat_id": {
                ...
                "title": "Unique identifier for the target chat ot username of target channel "
            ...
            "disable_notification": {
                "type": "boolean",
                "title": "Send the message silently. Users will receive a notification with no sound.",
            },
            "reply_to_message_id": {
                "type": "integer",
    ...
    def _send_notification(self, data: dict) -> Response:
        ...
        response, errors = request.post(url, json=data, path_to_errors=self.path_to_errors)
